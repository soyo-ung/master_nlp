...
{ # debut du premier for
	...
	status2=$(curl --silent -L --output ./PAGES-ASPIREES/"$cpttableau-$compteur".html --write-out "%{http_code}" $line);
	...
	then 
		...
		perl ./PROGRAMMES/minigrep/minigrepmultilingue.pl "utf-8" ./DUMP-TEXT/$cpttableau-$compteur.txt ./PROGRAMMES/minigrep/motif.txt ;
			...
			then
				...
				if [[ $VERIFENCODAGEDANSICONV == "" ]]
					then
						...
						perl ./PROGRAMMES/minigrep/minigrepmultilingue.pl "utf-8" ./DUMP-TEXT/$cpttableau-$compteur-utf8.txt ./PROGRAMMES/minigrep/motif.txt ;
						mv resultat-extraction.html ./CONTEXTES/$cpttableau-$compteur.html ;
						echo "ECRITURE RESULTAT dans le tableau" ;
